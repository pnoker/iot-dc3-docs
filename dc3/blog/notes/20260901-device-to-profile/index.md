...
> **配图**:`images/cover.png`（封面）、`images/concepts-skeleton.png`（四个概念，一副骨架）、`images/six-dimensions.png`（六个维度的差距）、`images/motor-steps.png`（把一台电机抽象成一套模板）、`images/storage-model.png`（存储模型关系图）、`images/command-pipeline.png`（指令下发的八态状态机）、`images/event-pipeline.png`（事件上报告警链路）、`images/config-objects.png`（五类对象，两条路径）、`images/semantic-ontology.png`（语义本体）。源文件在 `slides/` 同名前缀 HTML。

---

![封面：把设备抽象成一套语义模板](images/cover.png)
...
![四个概念，一副骨架](images/concepts-skeleton.png)
...
![语义模板与物模型：六个维度的差距](images/six-dimensions.png)
...
![把一台电机抽象成一套模板](images/motor-steps.png)
...
![存储模型：一个归属根，七张表](images/storage-model.png)
...
![指令下发：一条带状态机的异步链路](images/command-pipeline.png)
...
![事件上报：从一次发生到一条通知](images/event-pipeline.png)
...
![如何配置：五类对象，两条路径](images/config-objects.png)
...
![语义本体：面向智能体的建模接口](images/semantic-ontology.png)
