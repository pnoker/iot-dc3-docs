...
| **网络层**  | 现场总线、IoT 协议、无线 / WAN、消息传输 | 36 个协议驱动、RabbitMQ 异步消息总线、gRPC facade、南向读写命令通道                                    |
...
南向是另一套节奏：现场设备由协议驱动（`dc3-driver-*`，共 36 个）接入，驱动与数据中心之间**不直接调用**，而是经 RabbitMQ
...
- [模块地图](./modules) — Maven 模块结构、36 个驱动与依赖关系
