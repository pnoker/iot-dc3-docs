...
IoT DC3 的代码按"部署单元 + 共享契约 + 协议驱动"三类组织。这页从架构视角讲清楚：哪些模块会被打包成独立服务跑起来、它们靠哪些公共库与契约相互协作、36 个驱动如何按协议归类，以及驱动 SDK 暴露的 SPI 长什么样。读完你能定位任意一段功能落在哪个模块、它依赖谁。
...
| `dc3-driver-*`       | 协议适配（36 个独立进程）                         | 各自     | —      | 仅个别 |
...
36 个驱动是平台"协议广度"的载体。把它们按协议家族分组，比平铺一长串更容易找到你需要的那个。每个驱动是一个
