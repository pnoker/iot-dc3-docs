...
这组属性已在驱动注册时声明，但**当前写路径（`write()`）未被消费**——`write()` 读取的是位号属性（`point-attribute`），写寄存器/线圈的地址与功能码取自位号配置：
...
分支、直接返回 `false`（写失败）。因此写寄存器请把**位号的** `functionCode` 显式设为 `3`、写线圈设为 `1`，不要沿用默认 `6`。FC05/06/15/16 的语义尚未在代码里实现——以代码为准。
:::
