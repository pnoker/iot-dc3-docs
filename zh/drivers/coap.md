...
本驱动的**客户端读写**与**服务端上报接收**均为完整实现（非骨架），底层基于 Eclipse Californium。唯一未接通的是 CoAP **Observe** 订阅推送：`CoapObserveHandler` 仅是接口、无实现，因此驱动当前不能"订阅"资源由设备变更推送——需要事件驱动上报的场景请改用服务端
