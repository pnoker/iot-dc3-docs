...
const DICT = {
  zh: {
    aria: 'MySQL 驱动的请求-响应模型：采集调度按 cron 触发驱动经 JDBC 连接池向库发起 SELECT 查询，取结果第一行第一列作为位号值；写命令把值绑定进 UPDATE/INSERT 的 ? 占位符执行',
    modelNote: '请求-响应模型 · 驱动主动查询 · 库不会推送',
    platRegion: '平台侧 Platform · SQL 客户端',
    srcRegion: '数据源侧 · MySQL (业务库 / 历史库 / 第三方系统表)',
    schedName: '采集调度 Scheduler',
    schedSub: 'cron 0/30 * * * * ?',
    schedArrow: 'cron 触发',
    drvName: 'dc3-driver-mysql',
    drvSub: 'JDBC 客户端 · DRIVER_CLIENT',
    drvB1: '· mysql-connector-j (com.mysql.cj.jdbc.Driver)',
    ...
    qwSub: '? = 写入值 · 无 SQL 注入',
    dbName: 'MySQL 库',
    dbSub: 'MES/ERP · 第三方平台 · 历史归档',
    trName: 'TCP 3306 · MySQL 线协议',
    url1: 'jdbc:mysql://host:3306/iot',
    url2: '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC',
    exName: '示例 SQL (位号属性)',
    exR: 'read:   SELECT temperature FROM sensor WHERE id = 1',
    exW: 'write:  UPDATE sensor SET temperature = ? WHERE id = 1',
    exN: '空结果集 → null · 多行只取第一行 · 慢 SQL 应加索引',
  ...
  en: {
    aria: 'MySQL driver request/response model: the scheduler triggers the driver per cron to issue SELECT queries through its JDBC pool; the first row and first column of the result becomes the point value; writes bind the value into the ? placeholder of an UPDATE/INSERT',
    modelNote: 'request/response · the driver queries · the database never pushes',
    platRegion: 'Platform · SQL client',
    srcRegion: 'Data source · MySQL (business / history / 3rd-party tables)',
    ...
    dbSub: 'MES/ERP · 3rd-party · history archive',
    trName: 'TCP 3306 · MySQL wire protocol',
    url1: 'jdbc:mysql://host:3306/iot',
    url2: '?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC',
    exName: 'Example SQL (point attributes)',
    exR: 'read:   SELECT temperature FROM sensor WHERE id = 1',
    exW: 'write:  UPDATE sensor SET temperature = ? WHERE id = 1',
    exN: 'empty set → null · multi-row takes first · index slow SQL',
  ...
  <DiagramFrame>
    <div class="dc3-diagram">
      <svg :aria-label="s.aria" role="img" viewBox="0 0 1300 640">
        <defs>
          <marker id="mydb-ah" markerHeight="7" markerWidth="10" orient="auto" refX="9" refY="3.5">
            <polygon fill="var(--dc3-arrow)" points="0 0,10 3.5,0 7"/>
          </marker>
          <marker id="mydb-ah-rose" markerHeight="7" markerWidth="10" orient="auto" refX="9" refY="3.5">
            <polygon fill="var(--dc3-rose-stroke)" points="0 0,10 3.5,0 7"/>
          </marker>
          <pattern id="mydb-grid" height="40" patternUnits="userSpaceOnUse" width="40">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="var(--dc3-grid)" stroke-width="0.5"/>
          </pattern>
        </defs>
        <rect fill="url(#mydb-grid)" height="100%" width="100%"/>
        ...
        <!-- arrows -->
        <line marker-end="url(#mydb-ah)" stroke="var(--dc3-arrow)" stroke-width="1" x1="240" x2="240" y1="152"
              y2="192"/>
        <text fill="var(--dc3-arrow-label)" font-size="9" x="250" y="176">{{ s.schedArrow }}</text>

        <line marker-end="url(#mydb-ah)" stroke="var(--dc3-arrow)" stroke-width="1.2" x1="408" x2="612" y1="230"
              y2="230"/>
        ...
        <line marker-end="url(#mydb-ah)" stroke="var(--dc3-arrow)" stroke-width="1.2" x1="612" x2="408" y1="280"
              y2="280"/>
        ...
        <line marker-end="url(#mydb-ah-rose)" stroke="var(--dc3-rose-stroke)" stroke-dasharray="4,4" stroke-width="1"
        ...
        <line marker-end="url(#mydb-ah)" stroke="var(--dc3-arrow)" stroke-width="1" x1="240" x2="240" y1="500"
              y2="530"/>
        ...
        <text fill="var(--dc3-box-name)" font-size="10" font-weight="600" text-anchor="middle" x="940" y="466">{{
            s.cap
          }}
        </text>
        ...
        <line marker-end="url(#mydb-ah)" stroke="var(--dc3-arrow)" stroke-width="1.2" x1="330" x2="380" y1="610"
              y2="610"/>
        <text fill="var(--dc3-text2)" font-size="9" x="388" y="615">{{ s.legQuery }}</text>
        <line marker-end="url(#mydb-ah-rose)" stroke="var(--dc3-rose-stroke)" stroke-dasharray="4,4"
